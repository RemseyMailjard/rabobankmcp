...
Setup
------------------------------------------------------------------
    pip install fastmcp            # FastMCP 2.x
    python server.py               # runs over stdio

Inspect / test without a client:
    fastmcp dev server.py          # opens the MCP Inspector in the browser

Connect from a client (Claude Desktop / VS Code) by registering this server in
the client's MCP config and pointing it at:   python /path/to/server.py
"""
